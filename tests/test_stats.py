"""
Тесты статистического модуля ingestion/stats.py.

Проверяем, что онлайн-оценки (алгоритм Уэлфорда) совпадают с эталонными
из стандартной библиотеки, а детектор и матрица ошибок ведут себя
корректно на краевых случаях.
"""
import statistics
...
# ── RunningStats (алгоритм Уэлфорда) ───────────────────────────────────────
...
def test_running_stats_matches_stdlib():
    ...
    # выборочная дисперсия — делитель n-1
...
def test_running_stats_empty_and_single():
    ...
    rs.update(5.0)
    # одно наблюдение — дисперсии ещё нет
    assert rs.mean == 5.0
    assert rs.variance == 0.0
...
def test_running_stats_zscore():
    ...
    for x in [10, 10, 10, 10, 20, 10, 10, 10]:
        rs.update(x)
    # 20 должно давать заметно положительную Z-оценку
...
def test_iqr_bounds_flags_outlier():
    ...
    assert lower < 10
    assert upper > 19
    # явный выброс должен оказаться за верхней границей
    assert 1000 > upper
...
def test_detector_ignores_until_min_history():
    ...
    # пока истории мало — аномалий не объявляем, даже на крупной сумме
    for _ in range(4):
...
def test_detector_flags_clear_outlier():
    det = AnomalyDetector(z_threshold=3.0, min_history=5)
    # реалистичная история: небольшие суммы с естественным разбросом
    history = [10, 12, 9, 11, 13, 8, 10, 12, 9, 11,
               10, 14, 9, 12, 11, 10, 13, 8, 12, 10]
    for x in history:
        det.evaluate("user-1", float(x))
    # резкий выброс относительно истории пользователя
...
def test_detector_keeps_users_independent():
    ...
    # у нового пользователя своей истории нет — не аномалия
...
def test_confusion_matrix_counts_and_metrics():
    cm = ConfusionMatrix()
    cm.update(predicted=True, actual=True)    # TP
    cm.update(predicted=True, actual=True)    # TP
    cm.update(predicted=True, actual=False)   # FP
    cm.update(predicted=False, actual=True)   # FN
    cm.update(predicted=False, actual=False)  # TN
